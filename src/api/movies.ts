...
export const fetchMovieById:(id:number) => Promise<MovieDetails | null> = (id:number) => {
  const options = {
    method: 'GET',
    headers: {
      accept: 'application/json',
...
export const getMovieVideos = (id:string) => {
  ...
  return fetch(`https://api.themoviedb.org/3/mov/${id}/videos`, options).then(res => res.json())
}

export async function searchMovies (query: string, displayCount: number): Promise<MovieResult[]> {
  try {
    const options = {
      method: 'GET',
      headers: {
